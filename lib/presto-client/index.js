...
var QUERY_STATE_CHECK_INTERVAL = 100; // 100 ms default
var QUERY_STATE_MAX_CHECK_INTERVAL = 2500; // 2.5s
...
Client.prototype.statementResource = function(opts) {
  ...
  client.request(req, function(err, code, data){
    ...
    var fetch_next = function(next_uri, interval){
      /*
       * 1st time
{
 "stats": {
       ...
       * 2nd time
{
 ...
 "id": "20140120_032523_00000_32v8g"
      ...
      client.request(next_uri, function(error, code, response){
        ...
        if (response === undefined || response.stats === undefined) {
          error_callback(new Error('Got undefined response.stats for next_uri ' + next_uri));
          return;
        }
        ...
        if (response.stats.state === 'QUEUED'
          ...
          setTimeout(function(){ fetch_next(next, Math.min(QUERY_STATE_MAX_CHECK_INTERVAL, interval * 2)); }, interval);
          return;
        }
        ...
        if (response.nextUri) {
          var next = response.nextUri;
          setTimeout(function(){ fetch_next(next, 25); }, client.checkInterval);
          return;
        }
